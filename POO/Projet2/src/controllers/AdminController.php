...
class AdminController extends UserController{
    ...
    public function saveUser(){
        ...
        $validation = $this->validator->make($_POST+$_FILES, [
            ...
            'confirme-passU'     => 'required|same:passU',
            'photo'              => 'required'
        ],[
            'required' => ':attribute est requis',
            'nomU:min' => 'Minimum 2 caractères',
            'prenomU:min' => 'Minimum 2 caractères',
            'adresseU:min' => 'Minimum 4 caractères',
            'digits' => ':attribute doit avoir 9 chiffres',
            'numeric' => ':attribute est invalide',
            'salaireU:min' => 'Au moins 5 chiffres',
            'email' => ':attribute doit être un email',
            'loginU:min' => 'Minimum 5 caractères',
            'passU:min' => 'Minimum 5 caractères',
            'alpha'=> 'Uniquement des caractères alphabétiques',
            'alpha_num'=> 'Uniquement des caractères alpha-numériques',
            'same'     => 'Les passwords ne correspondent pas'
        ...
        if(!$validation->fails()){
            try {
                ...
                $this->user->setPhoto($_FILES["photo"]["name"]);
                if($this->savePhoto()){
                    $this->user->insert();
